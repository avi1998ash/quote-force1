...
export default class QuoteDetailCmp extends LightningElement {
   ...
   set recid(value) {
    ...
    connectedCallback() {
        ...
        // ── Dropdown positioning support (fix for clipped dropdowns) ────
        // The searchable Price Book / Price Book Entry dropdowns render
        // inside containers that scroll (the parent modal body, item
        // table rows, section cards). CSS `position: absolute` gets
        // clipped by any ancestor with `overflow` set, regardless of
        // z-index. Instead, the open dropdown panel is positioned with
        // `position: fixed` using coordinates measured from its trigger
        // button (see positionOpenDropdowns/positionDropdown below). This
        // only affects visual placement — no open/close state, search, or
        // selection logic is changed.
        this.resizeHandler = () => this.positionOpenDropdowns();
        window.addEventListener('resize', this.resizeHandler);

        // If the modal (or any other ancestor) scrolls while a dropdown is
        // open, close it rather than leaving it visually detached from its
        // trigger — reusing the exact same close logic already used for
        // outside clicks.
        this.scrollAncestor = this.findScrollableAncestor();
        this.ancestorScrollHandler = () => this.handleDocumentClick();

        if (this.scrollAncestor) {
            this.scrollAncestor.addEventListener(
                'scroll',
                this.ancestorScrollHandler,
                { passive: true }
            );
        }
    ...
    disconnectedCallback() {
        ...
        if (this.ancestorScrollHandler) {
            if (this.scrollAncestor) {
                this.scrollAncestor.removeEventListener(
                    'scroll',
                    this.ancestorScrollHandler
                );
            }
    ...
    renderedCallback() {
        this.positionOpenDropdowns();
    }

    findScrollableAncestor() {
        let el = this.template.host && this.template.host.parentElement;
    ...
    positionOpenDropdowns() {
        const dropdowns =
            this.template.querySelectorAll('.custom-combobox__dropdown');

        dropdowns.forEach((dropdown) => {
            const control = dropdown.parentElement;
            const trigger =
                control &&
                control.querySelector('.custom-combobox__trigger');
    ...
    positionDropdown(trigger, dropdown) {
        ...
        const minWidth = Math.min(288, viewportWidth - gutter * 2);
        const maxWidth = Math.min(448, viewportWidth - gutter * 2);

        let width = Math.max(triggerRect.width, minWidth);
        ...
        const estimatedHeight = dropdown.offsetHeight || 260;
        const spaceBelow = viewportHeight - triggerRect.bottom;
        const spaceAbove = triggerRect.top;

        if (spaceBelow < estimatedHeight + 12 && spaceAbove > spaceBelow) {
            dropdown.style.top =
                `${Math.max(gutter, triggerRect.top - estimatedHeight - 6)}px`;
                ...
                this.appendBundleSectionToUi(
                    {
                        ...
                        Name: this.selectedProdBundleName,
        ...
        .then(async data => {
            ...
            data.forEach(section => {
                const sectionObj = {
                    id: section.Id,
                    title: section.Name,
                    notes: section.QuoteForce__Notes__c,
                    total: 0,
                    products: []
                };

                const items = section.QuoteForce__QuoteLineItems__r || [];

                items.forEach(item => {
                    const line =
                        this.buildSectionProductRow({
                            id: item.Id,
                            prodId: item.QuoteForce__Product__c,
    ...
    async confirmDeleteItem() {
        ...
        try {
            ...
            console.error('[confirmDeleteItem] Failed:', error);
            this.showToast(
                'Error',
                this.normalizeError(error, 'Failed to delete item'),
                'error'
            );
        } finally {
                this.loaded = false;
    ...
    handleSectionNote(event) {
       ...
       buildSectionProductRow(row, sectionId) {
    const quantity = Number(row.Quantity) || 0;
    const price = Number(row.Price) || 0;

    const usedEntryIds = sectionId
        ? this.getUsedPriceBookEntryIdsInSection(sectionId, row.id)
        : new Set();
    const availableEntryOptions = (this.priceBookEntryOptions || []).filter(
        (option) => !usedEntryIds.has(option.value)
    );

    const normalizedSearchTerm = (row.entrySearchTerm || '').trim().toLowerCase();
    const filteredEntryOptions = normalizedSearchTerm
        ? availableEntryOptions.filter((option) => option.searchText.includes(normalizedSearchTerm))
        : [...availableEntryOptions];
    ...
    getUsedPriceBookEntryIdsInSection(sectionId, excludeRowId) {
        const used = new Set();
        const section = (this.sectionList || []).find(s => s.id === sectionId);
        (section?.products || []).forEach(product => {
            if (product.priceBookEntryId && product.id !== excludeRowId) {
                used.add(product.priceBookEntryId);
            }
        });
        return used;
    }
    ...
    event.stopPropagation();
    if (this.isSectionEntryPickerDisabled) return;
    ...
    this.sectionList = (this.sectionList || []).map(section => ({
        ...
        products: (section.products || []).map(product => {
            const isTarget = section.id === sectionId && product.id === rowId;
            if (!isTarget) {
                return product.isEntryDropdownOpen ? { ...product, isEntryDropdownOpen: false } : product;
            }
    ...
    const sectionId = sectionRecord.Id || this.generateTempId('section'); 

    const products = (selectedItems || []).map((item, index) =>
        this.buildSectionProductRow({
            id: insertedRows?.[index]?.Id || this.generateTempId('row'),
